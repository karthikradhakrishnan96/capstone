...
class RuleConfig:
    def __init__(self, rules, is_train=False):
        self.rules = rules
        self.is_train = is_train

    def __getitem__(self, index):
        return random.choice(self.rules[index]) #[:-2] if self.is_train else [self.rules[index][-1:-3:-1]])
...
# Change this later
# train_rule_config = RuleConfig(get_rule_texts(SUBREDDIT), is_train=True)
# val_rule_config = RuleConfig(get_rule_texts(SUBREDDIT))
train_rule_config = val_rule_config = RuleConfig(get_rule_texts(SUBREDDIT))
...
def processDialog(voc, dialog):
    ...
    for utterance in dialog.get_chronological_utterance_list():
        ...
        if utterance.meta['is_section_header']:
            continue
        tokens = encode(utterance.text, max_length=MAX_LENGTH)
        processed.append(
        ...
        if (split is None or convo.meta['split'] == split) and convo.get_utterance(convo.id).meta.get('subreddit',
            ...
            if is_train:
                ...
                if label == 1:
                    ...
                    pairs.append((context, reply, 1, convo.get_id() + "_" + comment_id + "_" + str(correct_rule_idx),
                                  encode(rule_config[correct_rule_idx])))
                    pairs.append((context, reply, 0, convo.get_id() + "_" + comment_id + "_" + str(wrong_rule_idx),
                                  encode(rule_config[wrong_rule_idx])))
                else:
                    correct_rule_idx = -1
                    wrong_rule_idx = get_wrong_rule_idx(correct_rule_idx, rule_config)
                    context = [u["tokens"] for u in dialog[:idx + 1]]
                    ...
                    pairs.append(
                        (context, reply, 0, convo.get_id() + "_" + comment_id + "_" + str(wrong_rule_idx2),
                         encode(rule_config[wrong_rule_idx2])))

            else:
                rule_config = val_rule_config
